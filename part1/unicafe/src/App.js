...
const Statistic = ({text, value, float, percent}) => {
  if (percent === "true") {
     return <tr><td>{text}</td><td>{value.toFixed(2)} %</td></tr>
  }
  return <tr><td>{text}</td><td>{value.toFixed(float)}</td></tr>
}
  ...
  return (
    <table>
      <tbody>
        ...
        <Statistic text="average" value={avg} float="2"/>
